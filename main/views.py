...
def contact(request):
  ...
  if request.method== 'POST':
    ...
    if form.is_valid:
      form.save()
      messages.success(request, 'your mrssage has been sent')
...
def profile_update(request):
  ...
  if request.method == 'POST':
    ...
    if profile.is_valid():
      ...
      messages.sucess(request, f'dear (new) your profile update is successful')
...
def add_to_cart(request):
  if request.method == 'POST':
      ...
      if cart:
        basket = Cart.objects.filter(user__username=request.user.username, paid=False, product=product.id, quantity=quantity,
        price = new_price).first()
        if basket:
...
def payment(request):
  if request.method == 'POST':
    ...
    api_key = 'sk_test_9361a4960f07d88f0854c5856bcb1199abb2ecee'
    curl = 'https://api.paystack.co/transaction/initialize'
    cburl = 'http:/44.203.126.89/callback'
    ref = str(uuid.uuid4())
    order_no = profile.id
    ...
    try:
        r = requests.post(curl,headers=header, json=data)
    except Exception:
...
def search(request):
    if request.method == 'POST':
        ...
        search = Product.object.filter(search_item)
